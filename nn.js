...
class EnhancedDerivTradingBot {
    ...
    analyzeTicksEnhanced(SecondToLastDigit, lastDigit) {
        if (this.tradeInProgress) {
            return;
        }

        const tickNumber = 5000
        // console.log(`${this.currentAsset} Recent ${tickNumber} tick History: ${this.tickHistory.slice(-tickNumber).join(', ')}`);
        // console.log(`${this.currentAsset} Recent ${tickNumber} tick History2: ${this.tickHistory2.slice(-tickNumber).join(', ')}`);

        // New method: analyze recent pairs of second-to-last vs last digits
        ...
        if (len === 0) {
            console.log('[Pair Analysis] No recent data to analyze.');
            return;
        }
        ...
        let longestRun = 0;
        let currentRun = 0;
        let totalConsecutivePairs = 0; // sum of (runLength - 1) for each run > 1
        // New counters for runs of length >= 2,3,4,5
        let runsGE2 = 0;
        let runsGE3 = 0;
        let runsGE4 = 0;
        let runsGE5 = 0;
        ...
        for (let i = 0; i < len; i++) {
            const sec = recentSecond[i];
            const last = recentLast[i];
            if (sec === last && last !== 0 && last !== 9 && last !== 3 && last !== 6 && last !== 2 && last !== 7) {
                totalMatches++;
                currentRun++;
                matchSamples.push(`${i}:${sec}|${last}`);
            } else {
                if (currentRun > 0) {
                    // accumulate pair counts
                    if (currentRun > 1) totalConsecutivePairs += (currentRun - 1);
                    // count runs by minimum length
        ...
        const winCalculation = ((totalMatches - totalConsecutivePairs) / 11) * this.currentStake;
        const lossCalculation = (totalConsecutivePairs * this.currentStake);
        ...
        //     winCalculation= $${winCalculation.toFixed(2)}
        //     lossCalculation= $${lossCalculation.toFixed(2)}
        //     profitLossCalculation= $${profitCalculation.toFixed(2)}
        // `);
        ...
        if ( SecondToLastDigit === lastDigit
            ...
            && this.lastDigit !== 2 
            && this.lastDigit !== 7
            // && this.lastDigit !== SecondToLastDigit 
            ) {
