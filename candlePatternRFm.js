...
class TelegramService {
  ...
  static async sendTradeAlert(type, symbol, direction, stake, duration, details = {}) {
    ...
    const msg = `
        ...
        ${type !== 'OPEN' ? `📉 xLosses x2-x9: ${state.session.x2Losses} | ${state.session.x3Losses} | ${state.session.x4Losses} | ${state.session.x5Losses} | ${state.session.x6Losses} | ${state.session.x7Losses} | ${state.session.x8Losses} | ${state.session.x9Losses}` : ''}
