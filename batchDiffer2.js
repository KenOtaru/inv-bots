...
class EnhancedDerivTradingBot {
    ...
    async loadPredictionOutcomes() {
        const filePath = path.join(__dirname, 'predictionOutcomes.json');
        try {
    ...
    analyzeTicksEnhanced() {
        ...
        const batchSizes = [50]; // Multi-scale analysis
        const minHistory = Math.min(...batchSizes); // Minimum ticks needed
        const lookbackWindow = 5000; // Focus on recent ticks
        ...
        // Analyze historical patterns for each batch size
        const maxCountsPerBatchSize = batchSizes.map(() => Array(10).fill(0));
        ...
        for (let b = 0; b < batchSizes.length; b++) {
            ...
            for (let d = 0; d < 10; d++) {
                const prefix = new Array(history.length + 1).fill(0);
                for (let i = 1; i <= history.length; i++) {
                    prefix[i] = prefix[i - 1] + (history[i - 1] === d ? 1 : 0);
                }
                for (let i = 0; i <= history.length - batchSize; i++) {
                    const c = prefix[i + batchSize] - prefix[i];
                    if (c > maxCountsPerBatchSize[b][d]) {
                        maxCountsPerBatchSize[b][d] = c;
                    }
                }
            }
        }

        // Combine max counts across batch sizes (weighted average)
        const weights = [1]; // Emphasize batchSize=10
        const maxCounts = Array(10).fill(0);
        for (let d = 0; d < 10; d++) {
            for (let b = 0; b < batchSizes.length; b++) {
                maxCounts[d] += weights[b] * maxCountsPerBatchSize[b][d];
            }
        }
        const globalHighestCount = Math.max(...maxCounts);

        // Identify digits that reached near the global max
        const globalBestDigits = [];
        for (let d = 0; d < 10; d++) {
            if (maxCounts[d] >= globalHighestCount * 0.9) { // Relaxed threshold
                globalBestDigits.push(d);
            }
        }

        if (globalHighestCount < 5) { // Lowered threshold for significance
            // console.log('No significant historical patterns detected. globalHighestCount:', globalHighestCount);
            // return;
        }

        // Store for inspection
        this.globalHighestCount = globalHighestCount;
        this.globalBestDigits = globalBestDigits.sort((a, b) => a - b);

        console.log('-------------------------------------------------');
        console.log('Global highest weighted count:', globalHighestCount.toFixed(2));
        console.log('Global best digits:', this.globalBestDigits);
        console.log('Max counts per digit:', maxCounts.map(c => c.toFixed(2)));
        console.log('-------------------------------------------------');

        // Analyze current batches for each batch size
        const currentCountsPerBatchSize = batchSizes.map(() => Array(10).fill(0));
        const entropies = [];
        for (let b = 0; b < batchSizes.length; b++) {
            const batchSize = batchSizes[b];
            const currentBatch = history.slice(-batchSize);
            if (currentBatch.length < batchSize) {
                console.log(`Current batch incomplete for size ${batchSize}:`, currentBatch.length);
                continue;
            }

            const counts = currentCountsPerBatchSize[b];
            ...
            let entropy = 0;
            for (let d = 0; d < 10; d++) {
                const p = counts[d] / batchSize;
                if (p > 0) entropy -= p * Math.log2(p);
            }
            entropies.push(entropy);
        }

        // Select the batch size with lowest entropy
        const bestBatchIdx = entropies.length > 0 ? entropies.indexOf(Math.min(...entropies)) : 1; // Default to batchSize=10 if none valid
        const bestBatchSize = batchSizes[bestBatchIdx];
        const currentCounts = currentCountsPerBatchSize[bestBatchIdx] || Array(10).fill(0);

        // if (!currentCounts.some(c => c > 0)) {
        //     console.log('No valid current counts for best batch size:', bestBatchSize);
        //     return;
        // }

        // Calculate exhaustion scores
        const exhaustionScores = Array(10).fill(0);
        const lastTicks = history.slice(-5); // Last 5 ticks for recency
        const streakCounts = Array(10).fill(0);
        for (const d of lastTicks) {
            if (typeof d === 'number' && d >= 0 && d <= 9) streakCounts[d]++;
        }

        for (let d = 0; d < 10; d++) {
            const currentCount = currentCounts[d];
            const historicalMax = maxCounts[d] || 1; // Avoid division by zero
            const recencyFactor = streakCounts[d] / 5; // 0 to 1
            // Simplified exhaustion: high if current count is high and near historical max
            exhaustionScores[d] = (currentCount / bestBatchSize) * 0.7 + 
                                (currentCount / historicalMax) * 0.2 + 
                                recencyFactor * 0.1;
            // console.log(`Digit ${d}: currentCount=${currentCount}, historicalMax=${historicalMax.toFixed(2)}, ` +
            //             `recencyFactor=${recencyFactor.toFixed(2)}, exhaustionScore=${exhaustionScores[d].toFixed(2)}`);
        }

        // Select digit with highest exhaustion score
        let predictedDigit = null;
        let maxExhaustion = 0;
        for (let d = 0; d < 10; d++) {
            if (exhaustionScores[d] > maxExhaustion && currentCounts[d] >= globalHighestCount * 0.7) { // Relaxed threshold
                maxExhaustion = exhaustionScores[d];
                predictedDigit = d;
            }
        }

        // if (predictedDigit === null) {
        //     // Fallback removed as requested; log why no digit was selected
        //     console.log('No digit selected: No exhaustion score meets criteria. Max exhaustion:', maxExhaustion);
        //     console.log('Current counts:', currentCounts);
        //     console.log('Global highest count:', globalHighestCount);
        //     return;
        // }
        ...
        // let confidence = 100 - (currentCounts[predictedDigit] / bestBatchSize * 100);
        // const outcomes = this.predictionOutcomes[predictedDigit];
        // if (outcomes.total > 0) {
        //     confidence = (confidence * 0.5) + (outcomes.wins / outcomes.total * 100 * 0.5);
        // }

        
        // console.log('Best batch size:', bestBatchSize, 'Entropy:', entropies[bestBatchIdx]?.toFixed(2) || 'N/A');
        // console.log('Predicted Digit Differ:', predictedDigit, 'Exhaustion Score:', maxExhaustion.toFixed(2), 'Confidence:', confidence.toFixed(2));
        
        // if (maxExhaustion < 0.6 && entropies[bestBatchIdx] < 3.5 && confidence > 84 && predictedDigit !== lastTick) { // Adjusted thresholds
        //     if(this.kTrade){
        ...
        // } else {
        //     this.kTrade = true;
        //     console.log('Skipping trade: Insufficient pattern strength or high entropy.');
        //     console.log('Max exhaustion:', maxExhaustion, 'Entropy:', entropies[bestBatchIdx]?.toFixed(2) || 'N/A');
        // }
        ...
        if(leastOccurringDigit !== this.lastDigit) {
            this.xDigit = leastOccurringDigit;
            this.winProbNumber = leastPercentage;
            console.log(`Placing Digit Differ trade on digit ${this.xDigit} (not expected to appear).`);
         this.placeTrade(this.xDigit, this.winProbNumber);
        }
    }
    ...
    async sendEmailSummary() {
        ...
        const mailOptions = {
            ...
            subject: '2Batch Differ Bot - Trading Summary',
            text: summaryText
        };
    ...
    async sendLossEmail() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: '2Batch Differ Bot - Loss Alert',
            text: summaryText
        };
    ...
    async sendErrorEmail(errorMessage) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: '2Batch Differ Bot - Error Report',
    ...
    async sendDisconnectResumptionEmailSummary() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: '2Batch Differ Bot - Status Update',
            text: summaryText
        };
