...
class EnhancedDerivTradingBot {
    ...
    async loadPredictionOutcomes() {
        const filePath = path.join(__dirname, 'gameThoery2.json');
        try {
    ...
    analyzeTicksEnhanced() {
        ...
        // Build transition matrix: from last digit to next digit counts
        const transitionCounts = Array.from({ length: 10 }, () => Array(10).fill(0));
        for (let i = 1; i < historyLen; i++) {
            const prev = history[i - 1];
            const next = history[i];
            if (typeof prev === 'number' && prev >= 0 && prev <= 9 &&
                typeof next === 'number' && next >= 0 && next <= 9) {
                transitionCounts[prev][next]++;
            }
        }

        // Compute transition probabilities
        const transitionProbs = Array.from({ length: 10 }, () => Array(10).fill(0));
        for (let prev = 0; prev < 10; prev++) {
            const total = transitionCounts[prev].reduce((sum, c) => sum + c, 0);
            if (total > 0) {
                for (let next = 0; next < 10; next++) {
                    transitionProbs[prev][next] = transitionCounts[prev][next] / total;
                }
            } else {
                // Uniform if no data
                for (let next = 0; next < 10; next++) {
                    transitionProbs[prev][next] = 0.1;
                }
            }
        }

        // console.log('Transition Probabilities (from prev to next):', transitionProbs.map(row => row.map(p => p.toFixed(3))));

        // Get last digit
        const lastDigit = history[historyLen - 1];
        if (typeof lastDigit !== 'number' || lastDigit < 0 || lastDigit > 9) {
            console.log('Invalid last digit:', lastDigit);
            return;
        }

        // Get probabilities for next digit given lastDigit
        const nextProbs = transitionProbs[lastDigit];

        // Calculate entropy of nextProbs to assess predictability
        let entropy = 0;
        for (let p of nextProbs) {
            if (p > 0) entropy -= p * Math.log2(p);
        }
        console.log('Next Digit Entropy:', entropy.toFixed(2));

        // Select digit with lowest probability (minimax: market minimizes your win by choosing likely, you max win by picking least likely)
        let predictedDigit = null;
        let minProb = Infinity;
        for (let d = 0; d < 10; d++) {
            if (nextProbs[d] < minProb) {
                minProb = nextProbs[d];
                predictedDigit = d;
            }
        }

        if (predictedDigit === null || minProb > 0.15) { // Threshold for sufficiently low probability
            console.log('No suitable low-probability digit. Min Prob:', minProb.toFixed(3));
            return;
        }
        ...
        // Ensure predicted digit differs from last tick (sequential game consideration)
        if (predictedDigit === lastDigit) {
            console.log('Skipping trade: Predicted digit matches last tick:', lastDigit);
            return;
        }

        // Calculate confidence: 1 - minProb, adjusted by historical outcomes
        let confidence = (1 - minProb) * 100;
        ...
        if (minProb < 0.15 && entropy < 3.31) { // Trade on low probability and predictable patterns
            ...
            console.log('-------------------------------------------------');
            console.log('Predicted Digit Differ:', predictedDigit, 'Probability:', (minProb * 100).toFixed(2), '%');
            console.log('Confidence:', this.winProbNumber, '%');
            console.log('Recent Ticks Entropy:', entropy.toFixed(2));
            console.log('-------------------------------------------------');

            console.log(`Placing Digit Differ trade on digit ${predictedDigit} (least likely to appear next).`);
            this.placeTrade(this.xDigit, this.winProbNumber);
        } else {
            console.log('Skipping trade: Probability not low enough or high entropy.');
            console.log('Min Prob:', (minProb * 100).toFixed(2), 'Entropy:', entropy.toFixed(2));
        }
    }
    ...
    async sendEmailSummary() {
        ...
        const mailOptions = {
            ...
            subject: '2GameTheory Differ Bot - Trading Summary',
            text: summaryText
        };
    ...
    async sendLossEmail() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: '2GameTheory Differ Bot - Loss Alert',
            text: summaryText
        };
    ...
    async sendErrorEmail(errorMessage) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: '2GameTheory Differ Bot - Error Report',
    ...
    async sendDisconnectResumptionEmailSummary() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: '2GameTheory Differ Bot - Status Update',
            text: summaryText
        };
