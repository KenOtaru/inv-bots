...
class EnhancedDigitDifferTradingBot {
    constructor(token, config = {}) {
        ...
        this.rStats = {};
        this.sys = 1;
        this.currentStrategyName = 'N/A';

        // // Initialize with equal weight
        // const strategyNames = Object.keys(this.predictionStrategies);
        // this.strategyWeights = {};
        // strategyNames.forEach(name => this.strategyWeights[name] = 1.0); 
    ...
    handleTickUpdate(tick) {
        ...
        // Define the pool of strategies the bot will rotate through.
        this.predictionStrategies = {
            leastOccurring: (history) => {
                const recentHistory = history.slice(-50);
                const digitCounts = Array(10).fill(0);
                recentHistory.forEach(digit => digitCounts[digit]++);
                let minCount = Infinity, prediction = 0;
            ...
            digitGap: (history) => {
                const lastOccurrence = Array(10).fill(-1);
                history.slice(-50).forEach((digit, index) => {
                    lastOccurrence[digit] = index;
                });
                let maxGap = -1, prediction = 0;
                lastOccurrence.forEach((pos, digit) => {
                    const gap = history.slice(-50).length - 1 - pos;
                    if (gap > maxGap) {
                        maxGap = gap;
                        prediction = digit;
                    }
            ...
            transitionAnalysis: (history) => {
                if (history.length < 20) return Math.floor(Math.random() * 10);
                const lastDigit = history[history.length - 1];
                const followers = Array(10).fill(0).map(() => Array(10).fill(0));
                for (let i = 1; i < history.length; i++) {
                    followers[history[i - 1]][history[i]]++;
                }
                const lastDigitFollowers = followers[lastDigit];
                let minCount = Infinity, prediction = 0;
                lastDigitFollowers.forEach((count, digit) => {
                    if (count < minCount) {
                        minCount = count;
                        prediction = digit;
                    }
    ...
    analyzeTicks(asset) {
        if (this.tradeInProgress || this.suspendedAssets.has(asset) || this.tickHistories[asset].length < this.config.requiredHistoryLength) {
            return;
        }
        ...
        Object.values(this.predictionStrategies).forEach(strategy => {
            const prediction = strategy(history);
            strategyResults.push(prediction);
            votes[prediction] = (votes[prediction] || 0) + 1;
        });

        let consensusDigit = null;
        let maxVotes = 0;
        for (const digit in votes) {
            if (votes[digit] > maxVotes) {
                maxVotes = votes[digit];
                consensusDigit = parseInt(digit);
            }
        }
        
        const consensusRatio = maxVotes / Object.keys(this.predictionStrategies).length;
        // --- END ENSEMBLE CONSENSUS LOGIC ---

        const lastDigit = history[history.length - 1];

        if (consensusDigit !== null && consensusDigit !== lastDigit && consensusRatio >= this.config.winProbabilityThreshold) {
            console.log(`[${asset}] CONSENSUS REACHED. Predicting digit: ${consensusDigit} with ${maxVotes}/${Object.keys(this.predictionStrategies).length} votes (${(consensusRatio*100).toFixed(0)}%).`);
            console.log(`  -> Individual predictions: [${strategyResults.join(', ')}]`);
            this.placeTrade(asset, consensusDigit);
        } else {
            console.log(`[${asset}] No consensus. Best was digit ${consensusDigit} with ${maxVotes} votes (${(consensusRatio*100).toFixed(0)}%). Threshold: ${(this.config.winProbabilityThreshold*100).toFixed(0)}%. Skipping trade.`);
            console.log(`  -> Individual predictions: [${strategyResults.join(', ')}]`);
        }
    }
    ...
    handleTradeResult(contract) {
        ...
        const adjustmentFactor = won ? 1.1 : 0.9;
        this.strategyWeights[this.currentStrategyName] *= adjustmentFactor;
        // Ensure weights don't get too close to zero
        if(this.strategyWeights[this.currentStrategyName] < 0.1) this.strategyWeights[this.currentStrategyName] = 0.1;
    ...
    async sendLossEmail(asset) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'GrokLF_Digit_Differ-Multi_Asset_Bot - Loss Alert',
            text: summaryText
        };
    ...
    async sendErrorEmail(errorMessage) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'GrokLF_Digit_Differ-Multi_Asset_Bot - Error Report',
