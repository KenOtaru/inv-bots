...
class EnhancedDerivTradingBot {
    ...
    analyzeTicksEnhanced() {
        ...
        // Need at least one full batch + next digit for analysis
        if (historyLen < batchSize + 1) return;

        // Step 1: Compute global percentages for each digit appearing as "next" after sliding batches of 100
        const globalP = new Array(10).fill(0);
        const numWindows = historyLen - batchSize;
        const nextCounts = new Array(10).fill(0);
        for (let i = 0; i < numWindows; i++) {
            const nextDigit = history[i + batchSize];
            nextCounts[nextDigit]++;
        }
        for (let d = 0; d < 10; d++) {
            globalP[d] = (nextCounts[d] / numWindows) * 100;
        }

        // Log global percentages
        console.log('-------------------------------------------------');
        console.log('Global next-digit percentages:', globalP.map(p => p.toFixed(2)));
        console.log('-------------------------------------------------');

        // Step 2: Analyze current batch (last 100 ticks)
        const currentStart = Math.max(0, historyLen - batchSize);
        const currentBatch = history.slice(currentStart);
        if (currentBatch.length < batchSize) return; // Wait for full current batch
        ...
        // Detect mode: streaking if high variance (e.g., max count > 1.2 * average)
        const avgCount = batchSize / 10;
        const maxCount = Math.max(...currentCounts);
        const isStreaking = (maxCount > 1.2 * avgCount);

        console.log('Current batch counts:', currentCounts);
        console.log('Mode:', isStreaking ? 'Streaking' : 'Balancing');

        // Step 3: Compute adjusted percentages with balancing/streaking adjustments
        const adjustedP = new Array(10).fill(0);
        const adjustmentFactor = 1; // Percentage points per unit deviation (tunable)
        for (let d = 0; d < 10; d++) {
            const deviation = currentCounts[d] - avgCount;
            let adjustment = adjustmentFactor * deviation;

            if (isStreaking) {
                // Streaking: High count -> more likely, low -> less likely
                adjustment = Math.abs(adjustment); // Invert sign for streaking
                if (deviation < 0) adjustment = -adjustment;
            } else {
                // Balancing: High count -> less likely, low -> more likely
                adjustment = -adjustment; // Negative for high, positive for low
            }

            adjustedP[d] = globalP[d] + adjustment;
            adjustedP[d] = Math.max(0, Math.min(100, adjustedP[d])); // Clamp to 0-100
        }

        // Step 4: Find digit with lowest adjusted percentage for Differ
        ...
        for (let d = 0; d < 10; d++) {
            if (adjustedP[d] < minP) {
                minP = adjustedP[d];
                predictedDigit = d;
            } else if (adjustedP[d] === minP && currentCounts[d] < currentCounts[predictedDigit]) {
                // Tiebreaker: Favor the one with lower current count for balancing
                predictedDigit = d;
            }
        }

        // Avoid repeating the same trade
    ...
    async sendEmailSummary() {
        ...
        const mailOptions = {
            ...
            subject: 'Batch Differ Bot - Trading Summary',
            text: summaryText
        };
    ...
    async sendLossEmail() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'Batch Differ Bot - Loss Alert',
            text: summaryText
        };
    ...
    async sendErrorEmail(errorMessage) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'Batch Differ Bot - Error Report',
    ...
    async sendDisconnectResumptionEmailSummary() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'Batch Differ Bot - Status Update',
            text: summaryText
        };
