...
class EnhancedDigitDifferTradingBot {
    ...
    analyzeTicks(asset) {
        ...
        // Green arc filter: Check max digit % in long window
        // const maxDigitInfo = this.getMaxFrequentDigit(longHistory);
        // if (maxDigitInfo.pct >= this.config.greenArcThreshold && maxDigitInfo.digit !== predicted) {
        //     console.log(`[${asset}] Green arc conflict. Adjusting... maxDigitInfo:`, maxDigitInfo);
        //     return;  // Skip betting if green arc conflict
        // }

         // Green arc filter: Check max digit % in long window
        // const minDigitInfo2 = this.getLeastFrequentDigit(longHistory);
        // if (maxDigitInfo.pct >= this.config.greenArcThreshold && maxDigitInfo2.digit !== predicted) {
        //     console.log(`[${asset}] Green arc conflict. Adjusting... minDigitInfo2:`, minDigitInfo2);
        //     return;  // Skip betting if green arc conflict
        // }
        ...
        if (repeatAnalysis) {
            ...
            if (highRepeatDigits.length > 0) {
                console.log(`⚠️ High repeat/sticky digits identified: ${highRepeatDigits.join(', ')}`);
            }
        }

        const repeatDigitStats = repeatAnalysis ? repeatAnalysis.digitStats[predicted] : null;
        console.log(`\n[${asset}] Digit Repeat Analysis (Digit: ${predicted} | Repeat Prob: ${repeatDigitStats.repeatProbability.toFixed(2)}%)`);
        ...
        if (
            ...
            // && this.lastDigits[asset] !== predicted
            && predicted === maxDigits[nPosition]
            ...
            && !highRepeatDigits.includes(predicted) // Double-check it's not in the high-risk list
        ) {
    ...
    async sendEmailSummary() {
        ...
        const mailOptions = {
            ...
            subject: 'HF_Multi_Asset_Bot - Summary',
            text: summaryText
        };
    ...
    async sendLossEmail(asset) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'HF_Multi_Asset_Bot - Loss Alert',
            text: summaryText
        };
    ...
    async sendErrorEmail(errorMessage) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'HF_Multi_Asset_Bot - Error Report',
