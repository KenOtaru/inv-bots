...
class EnhancedDerivTradingBot {
    constructor(token, config = {}) {
        ...
        this.predictionOutcomes2 = Array(10).fill(0).map(() => ({ wins: 0, total: 0 }));
    ...
    async savePredictionOutcomes2() {
        const filePath = path.join(__dirname, 'gameThoery.json');
        try {
            await fs.writeFile(filePath, JSON.stringify(this.predictionOutcomes2, null, 2));
    ...
    gameAnalyzer() {
        ...
        const outcomes = this.predictionOutcomes2[predictedDigit];
        if (outcomes.total > 0) {
    ...
    gameAnalyzer1() {
        ...
        // Need at least 10 ticks for meaningful game analysis
        if (historyLen < 10) return;

        // Step 1: Compute global transition probabilities (mixed strategies)
        // transitionP[lastDigit][nextDigit] = probability of nextDigit given lastDigit
        const transitionCounts = Array.from({ length: 10 }, () => new Array(10).fill(0));
        for (let i = 0; i < historyLen - 1; i++) {
            const last = history[i];
            const next = history[i + 1];
            transitionCounts[last][next]++;
        }

        const transitionP = Array.from({ length: 10 }, () => new Array(10).fill(0));
        for (let last = 0; last < 10; last++) {
            const total = transitionCounts[last].reduce((a, b) => a + b, 0);
            if (total > 0) {
                for (let next = 0; next < 10; next++) {
                    transitionP[last][next] = (transitionCounts[last][next] / total) * 100;
                }
            } else {
                // Uniform if no data
                transitionP[last].fill(10);
            }
        }

        // Step 2: Find global equilibrium frequencies (Nash-inspired stable probs)
        const globalCounts = new Array(10).fill(0);
        history.forEach(d => globalCounts[d]++);
        const globalP = globalCounts.map(count => (count / historyLen) * 100);

        // Log global equilibrium
        console.log('-------------------------------------------------');
        console.log('Global equilibrium probabilities:', globalP.map(p => p.toFixed(2)));
        console.log('-------------------------------------------------');

        // Step 3: Get current state (last 5 ticks for Tit-for-Tat context)
        const recentLen = Math.min(5, historyLen);
        const recent = history.slice(-recentLen);
        const lastDigit = recent[recent.length - 1];

        // Compute base next probabilities from transitions (mixed strategy)
        let nextP = [...transitionP[lastDigit]]; // Start with transitions from last digit

        // Adjust with Tit-for-Tat and balancing (Prisoner's Dilemma cooperation)
        const recentCounts = new Array(10).fill(0);
        recent.forEach(d => recentCounts[d]++);
        const avgRecent = recentLen / 10;

        for (let d = 0; d < 10; d++) {
            const deviation = recentCounts[d] - avgRecent;
            let adjustment = 0;

            // Tit-for-Tat: If digit appeared last, slightly boost (mirror); else, balance
            if (d === lastDigit) {
                adjustment += 5; // Encourage reciprocity
            } else if (deviation > 1) {
                adjustment -= 3; // Penalize over-representation (defection)
            } else if (deviation < -1) {
                adjustment += 3; // Boost under-representation (cooperate to balance)
            }

            // Blend with global equilibrium (Nash stability)
            nextP[d] = (nextP[d] * 0.7) + (globalP[d] * 0.3) + adjustment;
            nextP[d] = Math.max(0, Math.min(100, nextP[d])); // Clamp
        }

        // Step 4: Predict Differ on digit with lowest next probability
        let predictedDigit = null;
        let minP = Infinity;
        for (let d = 0; d < 10; d++) {
            if (nextP[d] < minP) {
        ...
        if (this.lastPredictedDigit === predictedDigit && this.lastPredictedBatchEnd === historyLen) {
            return;
        }
        ...
        // Trade if strong disequilibrium signal (low prob, high confidence)
        if (minP > 8.8 && confidence > 90) {
            this.xDigit = predictedDigit;
    ...
    batchAnalyzer() {
        ...
        const history = this.tickHistory || [];
        const historyLen = history.length;
        const batchSize = 100; // Fixed batch size of 100 ticks
        const lookbackWindow = 5000; // Recent ticks for historical analysis
        const minHistory = batchSize + 1; // Need at least one batch + next digit
        ...
        // Analyze historical next-digit transitions in batches of 100
        const recentHistory = history.slice(-Math.min(historyLen, lookbackWindow));
        const nextDigitCounts = Array(10).fill(0); // Count of each digit appearing after a batch
        let totalBatches = 0;

        for (let i = 0; i <= recentHistory.length - batchSize - 1; i += batchSize) {
            const batch = recentHistory.slice(i, i + batchSize);
            if (batch.length < batchSize) continue; // Skip incomplete batches
            const nextDigit = recentHistory[i + batchSize];
            if (typeof nextDigit === 'number' && nextDigit >= 0 && nextDigit <= 9) {
        ...
        if (totalBatches === 0) {
            console.log('No valid historical batches found.');
            return;
        }

        // Calculate next-digit probabilities
        const nextDigitProbs = nextDigitCounts.map(count => (count / totalBatches) * 100);
        console.log('Historical Next-Digit Probabilities (%):', nextDigitProbs.map(p => p.toFixed(2)));

        // Analyze current batch (last 100 ticks)
        const currentBatch = history.slice(-batchSize);
        if (currentBatch.length < batchSize) {
            console.log('Current batch incomplete:', currentBatch.length);
            return;
        }

        const currentCounts = Array(10).fill(0);
        for (const d of currentBatch) {
            if (typeof d === 'number' && d >= 0 && d <= 9) currentCounts[d]++;
        }

        // Calculate entropy for current batch
        let entropy = 0;
        for (let d = 0; d < 10; d++) {
            const p = currentCounts[d] / batchSize;
            if (p > 0) entropy -= p * Math.log2(p);
        }
        console.log('Current Batch Entropy:', entropy.toFixed(2));

        // Balance probabilities based on current batch distribution
        const meanCount = currentCounts.reduce((sum, c) => sum + c, 0) / 10;
        const adjustedProbs = nextDigitProbs.slice();
        for (let d = 0; d < 10; d++) {
            const count = currentCounts[d];
            // Boost probability for very low or very high counts
            const deviation = Math.abs(count - meanCount) / meanCount;
            if (count < meanCount * 0.5) {
                // Low appearance: slightly increase probability (mean reversion)
                adjustedProbs[d] *= (1 + deviation * 0.3);
            } else if (count > meanCount * 1.5) {
                // High appearance: slightly increase probability (persistence)
                adjustedProbs[d] *= (1 + deviation * 0.2);
            }
        }

        // Normalize adjusted probabilities to sum to 100%
        const probSum = adjustedProbs.reduce((sum, p) => sum + p, 0);
        if (probSum > 0) {
            for (let d = 0; d < 10; d++) {
                adjustedProbs[d] = (adjustedProbs[d] / probSum) * 100;
            }
        }
        console.log('Adjusted Next-Digit Probabilities (%):', adjustedProbs.map(p => p.toFixed(2)));
        console.log('Current Batch Counts:', currentCounts);

        // Select digit with lowest adjusted probability
        let predictedDigit = null;
        let minProb = Infinity;
        for (let d = 0; d < 10; d++) {
            if (adjustedProbs[d] < minProb) {
                minProb = adjustedProbs[d];
                predictedDigit = d;
            }
        }

        if (predictedDigit === null || minProb > 20) { // Threshold for low probability
            console.log('No suitable digit for prediction. Min probability:', minProb.toFixed(2));
            return;
        }
        ...
        // Calculate confidence (inverted probability, adjusted by historical outcomes)
        let confidence = 100 - minProb;
        ...
        if (minProb < 20 && entropy < 3.3 && confidence >= 100) { // Trade only on low probability and non-random patterns
            ...
            console.log('-------------------------------------------------');
            console.log('Predicted Digit Differ:', predictedDigit, 'Probability:', minProb.toFixed(2), '%');
            console.log('Confidence:', this.winProbNumber, '%');
            console.log('Current Batch Entropy:', entropy.toFixed(2));
            console.log('-------------------------------------------------');
            ...
            console.log('Skipping trade: Probability too high or entropy too high.');
            console.log('Min probability:', minProb.toFixed(2), 'Entropy:', entropy.toFixed(2));
        }
    }
    ...
    async sendEmailSummary() {
        ...
        const mailOptions = {
            ...
            subject: 'InvestBot Differ Bot - Trading Summary',
            text: summaryText
        };
    ...
    async sendLossEmail() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'InvestBot Differ Bot - Loss Alert',
            text: summaryText
        };
    ...
    async sendErrorEmail(errorMessage) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'InvestBot Differ Bot - Error Report',
    ...
    async sendDisconnectResumptionEmailSummary() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'InvestBot Differ Bot - Status Update',
            text: summaryText
        };
