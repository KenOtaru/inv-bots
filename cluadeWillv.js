...
class ConnectionManager {
    ...
    handleBuyResponse(response) {
        ...
        if (position) {
            ...
            console.log(`[DEBUG] Linked contract ${contract.contract_id} to position ${position.symbol} ${position.direction}`);
            LOGGER.info(`Linked contract ${contract.contract_id} to position ${position.symbol} ${position.direction}`);
        } else {
            console.log(`[DEBUG] Could not find position for reqId ${reqId}. Active reqIds:`, state.portfolio.activePositions.map(p => p.reqId));
