...
this._norm = {
  ...
  attract:   { mu: null, mad: null }
    ...
    analyzeTicksEnhanced() {
        ...
        const confidence = (analysis.confidence * 100).toString(); // Convert to percentage
        ...
        if(confidence.length > 4) {
          this.kTrade = true;
            console.log('Percentage Not Rounded:', confidence, '(', confidence.length,')');
            return;
        } else {
            console.log('Percentage Length:', confidence, '(', confidence.length,')');
        }
        ...
        if (this.consecutiveLosses < 1) {
           if (confidence > 60 && chaosRegim && !this.kChaos && this.kChaos !== null) {
             ...
             this.xKChaos = chaosRegim;
             this.placeTrade(analysis.predictedDigit, confidence);
           } else if (confidence > 60 && !chaosRegim && this.kChaos && this.kChaos !== null) {
             this.xDigit = analysis.predictedDigit;
             this.confidenceThreshold = confidence;
             this.antiAlgorithmScore = antiAlgorithmScore;
             this.chaosLevel = chaosLevel;
