...
this._norm = {
  ...
  attract:   { mu: null, mad: null }
    ...
    connect() {
        ...
        this.ws = new WebSocket('wss://ws.binaryws.com/websockets/v3?app_id=7612');
...
applyChaosTheory() {
  ...
  // consistent window: use last maxWindow points when available
  const windowed = history.length <= maxWindow ? history : history.slice(-maxWindow);

  // seed normalization states if not yet seeded (use statistics from window to reduce instance variance)
  const seedStats = (arr) => {
    if (!arr || arr.length === 0) return { mu: 0, mad: 1 };
    const sorted = arr.slice().sort((a,b)=>a-b);
    const mid = Math.floor(sorted.length/2);
    const median = (sorted.length % 2 === 1) ? sorted[mid] : (sorted[mid-1] + sorted[mid]) / 2;
    // mad
    const devs = sorted.map(v => Math.abs(v - median));
    const sortedDev = devs.slice().sort((a,b)=>a-b);
    const mad = (sortedDev.length % 2 === 1) ? sortedDev[Math.floor(sortedDev.length/2)]
              : ((sortedDev[Math.floor(sortedDev.length/2)-1] + sortedDev[Math.floor(sortedDev.length/2)])/2) || 1;
    return { mu: median, mad: Math.max(mad, 1e-6) };
  };

  // seed each normalization state if null (use different features' baseline but here we use same baseline)
  const baseline = windowed.slice(-Math.min(windowed.length, 100)); // last up-to-200 points
  if (this._norm.lyap.mu === null) Object.assign(this._norm.lyap, seedStats(baseline));
  if (this._norm.sens.mu === null) Object.assign(this._norm.sens, seedStats(baseline));
  if (this._norm.butterfly.mu === null) Object.assign(this._norm.butterfly, seedStats(baseline));
  if (this._norm.bifurc.mu === null) Object.assign(this._norm.bifurc, seedStats(baseline));
  if (this._norm.attract.mu === null) Object.assign(this._norm.attract, seedStats(baseline));

  // Lyapunov (Rosenstein-style)
  ...
  // Butterfly
  const butterflyStats   = this.detectButterflyEffects(windowed);
  const butterflyScore   = this.robustNormalize(butterflyStats.raw, this._norm.butterfly, 0.04);

  // Attractor
  const strangeAttractor = this.findStrangeAttractor(windowed);
  ...
  // Bifurcation
  const bifurcationStats = this.findBifurcationPoints(windowed);
...
calculateLyapunovExponent(history) {
    ...
    // Add deterministic tiny jitter only for distance comparisons (to break ties deterministically)
    // We'll not mutate X itself; use index-based tie-breaker when comparing distances.
    ...
    for (let i = 0; i < N - K; i += step) {
        ...
        for (let j = 0; j < N - K; j++) {
            ...
            // deterministic tiny tie-breaker based on j:
            const dTie = d + (j * 1e-12);
            if (d > 0 && dTie < best) { best = dTie; jStar = j; }
        }
...
detectButterflyEffects(history) {
  ...
  const start = Math.max(0, history.length - 50 - w);
...
calculateSensitivity(history) {
  ...
  for (let i = 0; i < N - K; i += step) {
    ...
    for (let j = 0; j < N - K; j++) {
      ...
      const d = this.euclid(X[i], X[j]);
      const dTie = d + (j * 1e-12);
      if (d > eps && dTie < best) { best = dTie; jStar = j; }
    }
    if (jStar === -1) continue;
...
// Robust adaptive normalization -> [0,1] centered ~0.5 at baseline
robustNormalize(raw, state, alpha = 0.05) {
  const eps = 1e-9;
  // if state not seeded (mu is null), set mu/mad already seeded earlier in applyChaosTheory.
  if (state.mu === null || state.mad === null) {
    // fallback to trivial seed
    state.mu = raw;
    ...
    // avoid mad becoming zero
    if (state.mad < 1e-12) state.mad = 1e-12;
  }
  ...
  return this.clamp(0.5 + 0.5 * Math.tanh(z));
    ...
    analyzeTicksEnhanced() {
        ...
        if (!chaosAnalysis) {
           // not enough data / no reliable chaos metrics yet
           console.log('Chaos analysis: insufficient data yet.');
           return;
        }
        ...
        console.log('Market in Chaos? :', chaosRegim, '(', this.regimCount, ')');

        const tickHistory2 = this.tickHistory.slice(-50);// Focus on last 20 ticks for recent pattern
        ...
        // Check if the predicted digit is different from the last digit and the xDigit
        if(leastPercentage < 7 && this.xLeastDigit !== leastOccurringDigit && this.xLeastDigit === this.lastDigit && this.xLeastDigit !== null ) {
    ...
    logTradingSummary() {
        ...
        console.log('xLeastDigit:', this.xLeastDigit);
    ...
    async sendEmailSummary() {
        ...
        const mailOptions = {
            ...
            subject: '2LF Deriv Differ Bot - Trading Summary',
            text: summaryText
        };
    ...
    async sendLossEmail() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: '2LF Deriv Bot - Loss Alert',
            text: summaryText
        };
    ...
    async sendErrorEmail(errorMessage) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: '2LF Deriv Bot - Error Report',
    ...
    async sendDisconnectResumptionEmailSummary() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: '2LF Deriv Bot - Status Update',
            text: summaryText
        };
