...
class QuantumInspiredDerivBot {
    constructor(token, config = {}) {
        ...
        this.kSystem = Math.floor(Math.random() * 5) + 1;  // Random digit 1 to 6
        // Round-robin random chooser: pick a random kSystem (1..5) different from current
        // and do not reuse a value until all others have been used.
        this.kChoices = [1, 2, 3, 4, 5];
        this._usedK = new Set();
        this.kSystem = this._pickNextKSystem();
    ...
    async analyzeAsset(asset) {
        ...
        //     Should Trade: ${decision7.shouldTrade ? 'Yes' : 'No'}
        //     Confidence: ${(decision7.confidence * 100).toFixed(2)}%
        //     inChaoticRegime: ${decision7.inChaoticRegime}
        //     chaosScore: ${decision7.chaosLevel}
        //     `);


        console.log('System:', this.kSystem);
        ...
        if (this.kSystem === 1) {
            ...
            trapLikelihood: ${decision.trapLikelihood}
            predictedDigit: ${decision.predictedDigit}
            `);
            
            console.log(` ${asset} Analysis:`);
            console.log('Should Trade:', decision6.shouldTrade, ':', decision.shouldTrade);

           if (decision.shouldTrade) {//decision7.shouldTrade && 
            console.log(`Active Analyses: `);
        ...
        } else if (this.kSystem === 2) {
            console.log(`Active Analyses: 
            Should Trade: ${decision2.shouldTrade ? 'Yes' : 'No'}
            Confidence: ${(decision2.confidence * 100).toFixed(2)}%
            Convergence: ${decision2.convergence}
            Fitness Score: ${(decision2.fitness * 100).toFixed(2)}%
            `);
            
            console.log(` ${asset} Analysis:`);
            console.log('Should Trade:', decision6.shouldTrade, ':', decision2.shouldTrade);

            if (decision2.shouldTrade) {//decision7.shouldTrade && 
            console.log(`Active Analyses: `);
            console.log(`n${asset} Tick: [${state.tickHistory.slice(-10).join(',')}]`);

            console.log(`\n🎯 ${asset} - SIGNAL DETECTED!`);
            console.log(`Confidence: ${(decision2.confidence * 100).toFixed(2)}%`);
            const currentDigit = state.tickHistory[state.tickHistory.length - 1];
            state.lastTradeTime = now;
            this.placeTrade(asset, currentDigit);
            }
        } else if (this.kSystem === 3) {
            console.log(`Active Analyses: 
            Should Trade: ${decision3.shouldTrade ? 'Yes' : 'No'}
            Confidence: ${(decision3.confidence * 100).toFixed(2)}%
            `);
            console.log(` ${asset} Analysis:`);
            console.log('Should Trade:', decision6.shouldTrade, ':', decision3.shouldTrade);

            if (decision3.shouldTrade) {//decision7.shouldTrade && 
            console.log(`Active Analyses: `);
            console.log(`n${asset} Tick: [${state.tickHistory.slice(-10).join(',')}]`);

            console.log(`\n🎯 ${asset} - SIGNAL DETECTED!`);
            console.log(`Confidence: ${(decision3.confidence * 100).toFixed(2)}%`);
            const currentDigit = state.tickHistory[state.tickHistory.length - 1];
            state.lastTradeTime = now;
            this.placeTrade(asset, currentDigit);
            }
        } else if (this.kSystem === 4) {
            console.log(`Active Analyses: 
            Should Trade: ${decision4.shouldTrade ? 'Yes' : 'No'}
            Confidence: ${(decision4.confidence * 100).toFixed(2)}%
            `);
            
            console.log(` ${asset} Analysis:`);
            console.log('Should Trade:', decision6.shouldTrade, ':', decision4.shouldTrade);

            if (decision4.shouldTrade && decision4.confidence < 0.05) {//decision7.shouldTrade && 
            console.log(`Active Analyses: `);
            console.log(`n${asset} Tick: [${state.tickHistory.slice(-10).join(',')}]`);

            console.log(`\n🎯 ${asset} - SIGNAL DETECTED!`);
            console.log(`Confidence: ${(decision5.confidence * 100).toFixed(2)}%`);
            const currentDigit = state.tickHistory[state.tickHistory.length - 1];
            state.lastTradeTime = now;
            this.placeTrade(asset, currentDigit);
            }
        } else if (this.kSystem === 5) {
            console.log(`Active Analyses: 
            Should Trade: ${decision5.shouldTrade ? 'Yes' : 'No'}
            Confidence: ${(decision5.confidence * 100).toFixed(2)}%
            Personality: ${decision5.personality}
            Due For Break: ${decision5.isDueForBreak}
            Pattern: ${decision5.streak}
            `);

            console.log(` ${asset} Analysis:`);
            console.log('Should Trade:', decision6.shouldTrade, ':', decision5.shouldTrade);

            if (decision5.shouldTrade && decision5.confidence > 0.9) {//decision7.shouldTrade && 
            console.log(`Active Analyses: `);
            console.log(`n${asset} Tick: [${state.tickHistory.slice(-10).join(',')}]`);
    ...
    connect() {
        try {
            ...
            this.ws = new WebSocket('wss://ws.binaryws.com/websockets/v3?app_id=1069');
    ...
    handleTradeResult(contract) {
        try {
            ...
            if(!won) {
                ...
                this.kSystem = Math.floor(Math.random() * 5) + 1;  // Random digit 1 to 5
                // choose a new kSystem using the no-repeat scheduler
                this.kSystem = this._pickNextKSystem();
            }
    ...
    // helper: pick next kSystem respecting "no repeat until all used" and different from current
    _pickNextKSystem() {
        // available choices exclude already used ones and the current kSystem
        let available = this.kChoices.filter(k => !this._usedK.has(k) && k !== this.kSystem);
        // if empty, reset used pool (but keep excluding current to avoid immediate repeat)
        if (available.length === 0) {
            this._usedK.clear();
            available = this.kChoices.filter(k => k !== this.kSystem);
        }
        const pick = available[Math.floor(Math.random() * available.length)];
        this._usedK.add(pick);
        return pick;
    }
    ...
    logTradingSummary() {
        ...
        console.log('Sytem', this.kSystem)
        console.log('═══════════════════════════\n');
    }
    ...
    async sendEmailSummary() {
        try {
            ...
            const mailOptions = {
                ...
                subject: 'nx2Differ Trading Report',
                text: summaryText
            };
    ...
    async sendLossEmail(asset) {
        try {
            ...
            const mailOptions = {
                from: this.emailConfig.auth.user,
                to: this.emailRecipient,
                subject: `nx2Differ Loss Alert - ${asset}`,
                text: summaryText
            };
    ...
    async sendFinalReport() {
        try {
            ...
            const mailOptions = {
                from: this.emailConfig.auth.user,
                to: this.emailRecipient,
                subject: 'nx2Differ Session Complete',
                text: summaryText
            };
